...
const ParentForm = ({ type, data }) => {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      const apiPayload = {
        ...formData,
        student_ids: formData.student_id,
      };
      delete apiPayload.student_id;
  ...
  const onStudentNameInputChange = async (e) => {
    ...
    try {
      const res = await api.post(`/students/searchStudent?search=${e}`);
      // Transform the data to match the expected format
      const mappedOptions = res.data.data.map((student) => ({
        value: student.id,
                  ...
                  <tr key={student.value}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      <input
                        ...
                        onChange={() => {
                          const newStudentIds = formData.student_id.includes(student.value)
                            ? formData.student_id.filter((id) => id !== student.value)
